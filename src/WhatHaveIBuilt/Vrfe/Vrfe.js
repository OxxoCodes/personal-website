...
const Vrfe = ({filename}) => {
  return (
    <BackCardWrapper filename={filename}>
      <div className="text">
        <h1>VRFE</h1>
        <p>VRFE, short for Virtual Reality Field Experiences, is a team I worked with as lead developer from roughly April 2021 to June
           2022. During this time, I was the lead developer of a cross-platform mobile VR Grand Canyon exploration simulation.</p>
        <p>As we wanted to reach so many target devices, from Oculus Quests to desktops to 2016 Android phones with 1GB of memory, there were some serious challenges that came about. The first of which was performance – when I arrived, performance on the existing application was subpar to say the least. After getting the application running on our target Android device, we were working with roughly 2 frames per second. As such, I focused on removing unused assets, simplifying geometry, optimizing sound files and textures, and refactoring a large amount of code. Thanks to these efforts, we had gone from an unusable application to one that hits a smooth 45 frames per second with ease on all devices, and requires 10x less storage space when built.</p>
