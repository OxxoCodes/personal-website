  ...
  return (
    <VrfeWrapper filename={filename}>
      <div className="text">
        ...
        <p>As we wanted to reach so many target devices, from Oculus Quests to desktops to 2016 Android phones with 1GB of memory, there were some serious challenges that came about. The first of which was performance – when I arrived, performance on the existing was subpar to say the least. After getting the application running on our target Android device, we were working with roughly 2 frames per second. Some serious work needed to be done. As such, I focused on removing unused assets, simplifying geometry, optimizing sound files and textures, and refactoring a large amount significant portion of code. Thanks to these efforts, we had gone from This changed an unusable application to one that hitting a smooth 45 frames per second with ease on all devices, and requiring 10x less storage space when built.</p>
